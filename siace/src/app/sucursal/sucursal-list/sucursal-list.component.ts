...
export class SucursalListComponent implements OnInit {
   displayedColumns = [ 'sucId',  'sucNombre',  'sucNumero',  'sucCalle',  'sucNumeroExterior',  'sucNumeroInterior',  'sucLatitud',  'sucLongitud',  'sucCiuId',  'sucColId',  'sucEmpId',  'actions'];
      ...
      confirmDialog.afterClosed().subscribe(result => {
         if (result === true) {
            ...
            this.sucursalService.delete(sucursal).subscribe({
               next: (result) => {
                  if (Number(result) > 0) {
                     this.toastr.success('El sucursal ha sido eliminado exitosamente', 'Transacción exitosa');
